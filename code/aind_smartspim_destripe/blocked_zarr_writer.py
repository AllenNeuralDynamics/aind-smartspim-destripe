...
def expand_chunks(
    ...
    if any(c > s for c, s in zip(chunks, data_shape)):
        msg = "chunks cannot be larger than data_shape in any dimension"
...
class BlockedArrayWriter:
    ...
    def gen_slices(
        ...
        """
        Generate a series of slices that can be used
        to traverse an array in blocks of a given shape.

        The method generates tuples of slices, each representing
        a block of the array. The blocks are generated by iterating
        over the array in steps of the block shape along each dimension.
        ...
        block_shape : tuple of int
            The desired shape of the blocks. This should be a tuple
            of integers representing the size of each dimension of
            the block. The length of `block_shape` should be equal
            to the length of `arr_shape`. If the array shape is not
            divisible by the block shape along a dimension,
        ...
        if len(arr_shape) != len(block_shape):
            msg = "array shape and block shape have different lengths"
            raise Exception(msg)

        def _slice_along_dim(dim: int) -> Generator:
            """
            A helper generator function that slices
            along one dimension.
            """
            # Base case: if the dimension is beyond the
            # last one, return an empty tuple
    ...
    @staticmethod
    def store(in_array: da.Array, out_array: ArrayLike, block_shape: tuple):
        """
        Partitions the last 3 dimensions of a Dask array
        into non-overlapping blocks and writes them sequentially
        to a Zarr array. This is meant to reduce the scheduling burden
        ...
        """
        # Iterate through the input array in steps equal
        # to the block shape dimensions
    ...
    def get_block_shape(arr, target_size_mb=409600, mode="cycle"):
        """
        Given the shape and chunk size of a pre-chunked array,
        determine the optimal block shape closest to target_size.
        Expanded block dimensions are an integer multiple
        of the chunk dimension to ensure optimal access patterns.
